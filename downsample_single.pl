...
## Usage
my $usage = "
downsample_single.pl  -  Randomly downsamples paired fastq files.  Outputs
                         gzipped fastq files.

usage:  downsample_single.pl \\
        [options] \\
        <read file [fastq or fastq.gz]>
...
options:
  ...
  -l    Average length of reads
        If no value given, the average read length will be calculated (takes longer)
        If read length is given, be aware that the script does not check whether they are accurate.
  -r    Number of reads to output
        If -r is given, -g, -f, and -l will be ignored.
        If -r is not given, values must be entered for -g and -f
  -o    Force output of all reads into new file if number of reads in original file is less than
...
my $count1;
if ($in){
    print STDERR "\tCalculating read lengths ...";
    my @lengs;
    while (<$in>){
        my $id = $_;
        chomp(my $seq = <$in>);
...
if (!$numseq){
    my ($leng1) = $rleng;
    $numseq = int(($gensize * $fold * 1000000)/($leng1));
...
my $index = 0;
open (my $out1, " | gzip -c - > $pref.fastq.gz") or die "Can't open output file 1: $!\n";
